...
if (isset($_FILES['image'], $_POST['title'], $_POST['description'])) {
	...
	if (!empty($_FILES['image']['tmp_name']) && getimagesize($_FILES['image']['tmp_name'])) {
		if (file_exists($image_path)) {
			...
			if ($stmt = $con->prepare('SELECT filepath FROM profile_images WHERE userid = ? AND profile_image = 1')) {
				...
				if ($stmt->num_rows > 0) { //If the profile image is already exists
					...
					// The path of the new uploaded image
					$image_path = $target_dir . $rename_image_name . basename($_FILES['image']['name']);
					$stmt = $con->prepare('UPDATE profile_images SET title = ?, description = ?, filepath = ?, uploaded_date = CURRENT_TIMESTAMP WHERE userid = ? AND profile_image = 1');
					...
					// The path of the new uploaded image
					$image_path = $target_dir . $rename_image_name . basename($_FILES['image']['name']);
					$stmt = $con->prepare('INSERT INTO profile_images (userid, title, description, filepath, uploaded_date, profile_image) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, 1)');
